...
behind examexam's tkinter GUI so that contributors can extend or modify it
confidently.
...
```bash
uv run examexam gui        # subcommand
uv run examexam --gui      # top-level flag
uv run examexam-gui        # gui-scripts entry point (pyproject.toml)
```

All three converge on `examexam.gui.app:launch_gui`.
...
if args.command == "gui" or getattr(args, "gui", False):
    from examexam.gui.app import launch_gui
...
def _fetch(scope_str):
    ...
    from examexam.services import read_current_report
...
1. Add a sidebar entry in `examexamApp._build_ui`:
...
```python
with patch("examexam.services.read_current_report",
