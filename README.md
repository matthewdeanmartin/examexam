...
1. **(Optional) Customize the prompts**
...
1. Environment variables (e.g., `EXAMEXAM_GENERATE_MODEL=...`)
1. Values in `examexam.toml`
...
1. `--model-provider`: Chooses a provider like `openai`, `anthropic`, `google`, etc.
1. `--model-class`: Use either `fast` (default, for cheaper, quicker models) or `frontier` (for more powerful, expensive models). Used in combination with `--model-provider`.
