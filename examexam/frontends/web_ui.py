...
class BrowserSession:
    ...
    def has_active_exam(self) -> bool:
        return bool(
            self.test_name
            and self.question_file
            and self.start_time
            and self.question_order
        )

    def completed_count(self) -> int:
        return sum(
            1 for question in self.session if question.get("user_score") is not None
        )
...
class WebUI:
    ...
    def configure_take_exam(self, question_file: str | None) -> None:
        ...
        self._configured_question_file = (
            Path(question_file).resolve() if question_file else None
        )
    ...
    def show_answer_feedback(self, feedback: AnswerFeedback) -> None:
        self.show_message(
            "Answer submitted.", style="success" if feedback.is_correct else "warning"
        )

    def show_results(self, result: ExamResult) -> None:
        label = "Passed" if result.passed else "Completed"
        self.show_message(
            f"{label}: {result.score}/{result.total} ({result.percent:.1f}%)",
            style="info",
        )
    ...
    def progress_start(self, total: int, description: str = "") -> str:
        ...
        self._progress_items[task_id] = ProgressInfo(
            task_id=task_id, description=description, total=total, current=0
        )
        return task_id
    ...
    def _setup_routes(self) -> None:
        ...
        def index(
            ...
            if created_session_id:
                page.set_cookie(
                    COOKIE_NAME, created_session_id, httponly=True, samesite="lax"
                )
            return page
        ...
        async def start_exam(
            ...
            if resume_requested and selected_test.resume_info is not None:
                session_data, saved_start_time = self._load_saved_session(
                    selected_test.name
                )
                ...
                session = copy.deepcopy(
                    load_questions(str(selected_test.question_file))
                )
            ...
            save_session_file(
                get_session_path(selected_test.name),
            ...
            destination = (
                "/results"
                if browser_session.completed_count()
        ...
        def question_page(
            ...
            question, session_question = self._question_pair_for_number(
                browser_session, n
            )
            if (
                session_question.get("start_time") is None
                and browser_session.start_time is not None
            ):
        ...
        async def submit_answer(
            ...
            form = await request.form()
            submitted_answers = [
                value for value in form.getlist("answer") if isinstance(value, str)
            ]
            ...
            response = RedirectResponse(
                url=f"/feedback/{n}", status_code=status.HTTP_303_SEE_OTHER
            )
            if created_session_id:
                response.set_cookie(
                    COOKIE_NAME, created_session_id, httponly=True, samesite="lax"
        ...
        def feedback_page(
            ...
            if session_question.get("user_score") is None:
                response = RedirectResponse(
                    url=f"/question/{browser_session.completed_count() + 1}",
            ...
            selected = self._selected_options(
                display_options, session_question.get("user_answers", [])
            )
            feedback = build_answer_feedback(display_options, selected)
            page = self._render(
                "feedback.html",
                ...
                feedback=feedback,
                option_details=self._feedback_option_details(
                    display_options, session_question
                ),
                next_url=(
                    "/results"
                    if browser_session.completed_count()
        ...
        def next_question(
            ...
            elif browser_session.completed_count() >= len(
                browser_session.question_order
            ):
                response = RedirectResponse(
                    url="/results", status_code=status.HTTP_303_SEE_OTHER
                )
            else:
                response = RedirectResponse(
        ...
        def mark_defective(
            ...
            if browser_session.has_active_exam:
                ...
                browser_session.flash_message = (
                    "This question has been marked as defective in the session file."
                )
        ...
        def results_page(
            ...
            if browser_session.start_time is None:
                raise HTTPException(
                    ...
                    detail="Missing exam start time.",
                )
        ...
        def progress_page(
            ...
            page = self._render(
                ...
                progress_items=sorted(
                    self._progress_items.values(), key=lambda item: item.description
                ),
            )
            if created_session_id:
                page.set_cookie(
    ...
    def _read_resume_info(self, test_name: str) -> ResumeInfo | None:
        ...
        completed = sum(
            ...
            if question.get("user_score") is not None
        )
    ...
    def _load_saved_session(
        ...
        parsed_start = (
            datetime.fromisoformat(start_time) if isinstance(start_time, str) else None
        )
        return list(data.get("questions", [])), parsed_start

    def _build_question_order(
        self, questions: list[dict[str, Any]], session: list[dict[str, Any]]
    ) -> list[str]:
    ...
    def _pop_flash(self, browser_session: BrowserSession) -> dict[str, str] | None:
        if browser_session.flash_message is None:
            return None
        payload = {
            "message": browser_session.flash_message,
            "level": browser_session.flash_level,
        }
        browser_session.flash_message = None
        browser_session.flash_level = "info"
        return payload

    def _validate_question_route(
    ...
    def _question_pair_for_number(
        ...
        question_map = {
            question.get("id", ""): question for question in browser_session.questions
        }
        session_map = {
            question.get("id", ""): question for question in browser_session.session
        }
    ...
    def _display_options(
        self, browser_session: BrowserSession, question: dict[str, Any]
    ) -> list[dict[str, Any]]:
        question_id = str(question.get("id", ""))
    ...
    def _selected_options(
        ...
        return [
            ...
            if option.get("text") in selected_answer_set
        ]
    ...
    def _question_context(self, question: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "question": self._format_question_text(
                question_model.question, question_model.answer_count
            ),
        }

    def _option_context(
        self, display_options: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
