...
def generate_study_guide(
    topic: str, model: str, ui: FrontendUI | None = None
) -> str | None:
    ...
    if ui:
        ui.show_message(
            f"Generating study guide for topic: {topic} using model {model}..."
        )
    ...
    if not content:
        if ui:
            ui.show_error(
                "Failed to generate study guide. The model returned no content."
            )
        return None
...
def generate_topic_research_now(
    topic: str, model: str = "openai", ui: FrontendUI | None = None
) -> None:
